...
public:
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return GetKarazhanAI<npc_mana_feederAI>(creature);
    }
    struct npc_mana_feederAI : public ScriptedAI
    {
        npc_mana_feederAI(Creature* creature) : ScriptedAI(creature)
        {
            pInstance = creature->GetInstanceScript();
        }
        InstanceScript* pInstance;
        ObjectGuid ArcaneProtectorGUID;
...
public:
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return GetKarazhanAI<npc_arcane_protectorAI>(creature);
    }
    struct npc_arcane_protectorAI : public ScriptedAI
    {
        npc_arcane_protectorAI(Creature* creature) : ScriptedAI(creature)
        {
            pInstance = creature->GetInstanceScript();
        }
        InstanceScript* pInstance;
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (summonTimer <= diff)
            {
                Creature* ArcaneProtector = me->SummonCreature(NPC_ASTRAL_SPARK, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000s);
                ArcaneProtectorGUID = ArcaneProtector->GetGUID();
                Creature* arcanepro = ObjectAccessor::GetCreature(*me, ArcaneProtectorGUID);
